...
Main goal is to make DM Editor servered as "service / library for visual editing and rendering" in PHP, with use cases like making compaign page visually. Technical benefit would be, being able to combining flexibility of PHP template and DM Editor's visual widget(eg. interactive widget tabs, collapsable text, pop up) and all render only once (no converting needed).
